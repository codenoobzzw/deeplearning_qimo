...
if [ ! -d data/calvin/lerobot_env_A ]; then
  echo "Missing data/calvin/lerobot_env_A. Run scripts/prepare_calvin_splits.py after dataset metadata is available." >&2
  exit 1
fi

bash scripts/run_and_log.sh act_A_train outputs/task2/act_A \
  bash -lc "PYTHONPATH='$LEROBOT_PYTHONPATH' $PYTHON_BIN -m lerobot.scripts.lerobot_train --config_path configs/act_A.yaml"
